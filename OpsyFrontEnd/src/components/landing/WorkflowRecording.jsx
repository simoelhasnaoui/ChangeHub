...
export default function WorkflowRecording() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      while (!stopped) {
        setProgress(5); showPage('wr-login'); setUrl('ChangeHub.app/login')
        setLabel('Requester logs in')

        await moveTo(cursor, 128, 195); await click(ring)
        await typeInto('wr-email', 'simo@dxc.com', 45)
        ...
        if (stopped) break
        setProgress(18); showPage('wr-requester'); setUrl('ChangeHub.app/requester/new')
        ...
        $('wr-type').textContent = 'Deployment'; $('wr-type').style.color = '#fefce8'
        ...
        $('wr-impl').textContent = 'Oussama M.'; $('wr-impl').style.color = '#fefce8'
        ...
        if (stopped) break
        setProgress(42); showPage('wr-approver'); setUrl('ChangeHub.app/approver')
        ...
        if (stopped) break
        setProgress(68); showPage('wr-implementer'); setUrl('ChangeHub.app/implementer')
        setLabel('Implementer picks up the task'); await wait(600)

        await moveTo(cursor, 215, 196, 700); await click(ring)
        $('wr-ibadge').style.background = 'rgba(99, 102, 241, 0.2)'; $('wr-ibadge').style.color = '#818cf8'; $('wr-ibadge').style.border = '1px solid rgba(99, 102, 241, 0.5)'
        ...
        setProgress(100)
        $('wr-audit').innerHTML = ''
        addAudit('Draft created — Simo B.', 'rgba(254, 252, 232, 0.3)')
        addAudit('Submitted for approval', '#D5CBE5')
        addAudit('Approved by Saad L.', '#4ade80')
        addAudit('Started by Oussama M.', '#818cf8')
        addAudit('Marked done — no incidents', '#4ade80')
        showPage('wr-done'); setUrl('ChangeHub.app/implementer')
        ...
        $('wr-ibadge').style.background = 'rgba(92, 45, 143, 0.2)'; $('wr-ibadge').style.color = '#e879f9'; $('wr-ibadge').style.border = '1px solid rgba(92, 45, 143, 0.5)'
  ...
  const field = { border: '1px solid rgba(92, 45, 143, 0.5)', borderRadius: '6px', padding: '5px 8px', fontSize: '11px', background: 'rgba(62, 30, 112, 0.4)', minHeight: '26px', lineHeight: 1.4, color: '#fefce8' }
  ...
  return (
    <div ref={ref} style={{ transform: 'scale(1.45)', transformOrigin: 'center center', width: '100%', display: 'flex', justifyContent: 'center' }}>
      <style>{`
        ...
        @keyframes wrRipple{0%{transform:scale(0);opacity:.6}100%{transform:scale(2.5);opacity:0}}
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ background: 'rgba(62, 30, 112, 0.4)', border: '1px solid rgba(92, 45, 143, 0.5)', borderRadius: 10, overflow: 'hidden', backdropFilter: 'blur(24px)' }}>
          <div style={{ background: 'rgba(59, 7, 100, 0.4)', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid rgba(92, 45, 143, 0.5)' }}>
            ...
            <div className="wr-urlbar" style={{ flex: 1, background: 'rgba(62, 30, 112, 0.5)', border: '1px solid rgba(92, 45, 143, 0.3)', borderRadius: 5, padding: '3px 10px', fontSize: 11, color: 'rgba(254, 252, 232, 0.7)' }}>ChangeHub.app/login</div>
          </div>
          ...
          <div style={{ position: 'relative', overflow: 'hidden', height: 340 }}>
            ...
            <div id="wr-login" className="wr-page wr-active" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', background: 'transparent' }}>
              <div style={{ background: 'rgba(62, 30, 112, 0.4)', border: '1px solid rgba(92, 45, 143, 0.5)', borderRadius: 10, padding: '24px 28px', width: 260 }}>
                <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 2, color: 'transparent', backgroundImage: 'linear-gradient(to right, #D5CBE5, #816A9E)', WebkitBackgroundClip: 'text' }}>ChangeHub</div>
            ...
            <div id="wr-requester" className="wr-page" style={{ background: 'transparent', display: 'flex' }}>
              <div style={sidebar}>
                <div style={{ padding: '0 12px', fontWeight: 600, fontSize: 13, marginBottom: 10, color: '#fefce8' }}>ChangeHub</div>
              ...
              <div style={{ flex: 1, padding: 16 }}>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10, color: '#fefce8' }}>Nouvelle demande</div>
            ...
            <div id="wr-approver" className="wr-page" style={{ background: 'transparent', display: 'flex' }}>
              ...
              <div style={{ flex: 1, padding: 16 }}>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10, color: '#fefce8' }}>Demandes en attente</div>
                <div style={{ background: 'rgba(62, 30, 112, 0.3)', border: '1px solid rgba(92, 45, 143, 0.5)', borderRadius: 6, padding: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: '#fefce8' }}>Deploy v2.4 — Payment...</div>
                    <div style={{ ...badge, background: 'rgba(213, 203, 229, 0.2)', color: '#D5CBE5', border: '1px solid rgba(213, 203, 229, 0.5)' }}>En attente</div>
                  </div>
                  <div style={{ fontSize: 10, color: 'rgba(254, 252, 232, 0.7)', marginBottom: 8 }}>Demandeur: Simo B. • Risque: Medium</div>
                  ...
                  <div id="wr-apanel" style={{ display: 'none', borderTop: '1px dashed rgba(92, 45, 143, 0.5)', paddingTop: 8, marginTop: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <div style={{ ...btn, background: 'rgba(239, 68, 68, 0.2)', color: '#f87171', border: '1px solid rgba(239, 68, 68, 0.5)', flex: 1 }}>Rejeter</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div id="wr-implementer" className="wr-page" style={{ background: 'transparent', display: 'flex' }}>
              ...
              <div style={{ flex: 1, padding: 16 }}>
                <div style={{ background: 'rgba(62, 30, 112, 0.3)', border: '1px solid rgba(92, 45, 143, 0.5)', borderTop: '3px solid #D5CBE5', borderRadius: 6, padding: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#fefce8' }}>Deploy v2.4</div>
                    <div id="wr-ibadge" style={{ ...badge, background: 'rgba(92, 45, 143, 0.2)', color: '#e879f9', border: '1px solid rgba(92, 45, 143, 0.5)' }}>Approuvé</div>
                  </div>
                  <div style={{ fontSize: 10, color: 'rgba(254, 252, 232, 0.5)', marginBottom: 8 }}>Approuvé par: Saad L.</div>
            ...
            <div id="wr-done" className="wr-page" style={{ background: 'transparent', display: 'flex' }}>
              ...
              <div style={{ flex: 1, padding: 16 }}>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10, color: '#fefce8' }}>Piste d'audit: Deploy v2.4</div>
                <div style={{ background: 'rgba(62, 30, 112, 0.3)', border: '1px solid rgba(92, 45, 143, 0.5)', borderRadius: 6, padding: 12 }}>
                  <div id="wr-audit" style={{ fontSize: 10, color: '#fefce8', display: 'flex', flexDirection: 'column', gap: 6, fontFamily: 'monospace' }} />
                </div>
              </div>
            </div>
